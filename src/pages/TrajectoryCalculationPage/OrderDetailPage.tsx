...
export const TrajectoryCalculationPage: FC = () => {
    ...
    useEffect(() => {
        if (id) {
            dispatch(loadCurrentRequest(parseInt(id)))
        }
    }, [id, dispatch])
    ...
    const handleRemoveComet = async (cometId: number) => {
        if (window.confirm('Удалить эту комету из заявки?')) {
            try {
    ...
    const handleConfirmRequest = async () => {
        if (window.confirm('Подтвердить заявку? После подтверждения редактирование будет невозможно.')) {
            try {
    ...
    return (
        <>
            <Breadcrumbs crumbs={[
                ...
                { label: `Заявка #${currentRequest.id}` }
            ]} />
                                            ...
                                            <div className="item-actions">
                                                {editingComet === item.id ? (
                                                    ...
                                                    <>
                                                        ...
                                                        <button
                                                            className="btn btn-small btn-red"
                                                            onClick={() => handleRemoveComet(item.id)}
                                                            disabled={loading}
                                                        >
                                                            Удалить
                                                        </button>
